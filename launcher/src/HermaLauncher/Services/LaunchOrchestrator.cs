...
public sealed class LaunchOrchestrator
{
    ...
    public async Task<Process?> RunAsync(LaunchOptions options, IProgress<StageUpdate> progress, CancellationToken ct)
    {
        try
        {
            ...
            // 채널(설정): 정식=26.1.2 Fabric 자동접속 / 베타=26.1.2 Fabric / RPG=1.21.1 NeoForge.
            //   비-정식(beta/rpg)은 서버 상태 미동기화라 멀티 자동접속/서버목록 등록 생략(게임 실행까지만 — 싱글플레이 테스트).
            //   ※ 로더/MC버전 분기는 CmlLibMinecraftService.EnsureJavaAsync 가 동일 채널을 읽어 처리(Fabric/NeoForge).
            ...
            await _packwiz.RunAsync(javaPath, channel.PackTomlUrl, progress, ct).ConfigureAwait(false);
            ct.ThrowIfCancellationRequested();
            ...
            //   비-정식 채널: 자동접속 미사용이라 해석 생략(미사용 폴백 endpoint).
            ServerEndpoint endpoint;
            if (!channel.AutoConnect)
            {
                progress.Report(StageUpdate.Of(LaunchStage.Launch,
                    "테스트 채널 — 멀티 서버 자동접속은 생략해요(서버 준비 전). 게임이 뜨면 싱글플레이로 새 모드를 확인하세요."));
            ...
            //   비-정식: registerServer:false 로 servers.dat 등록만 생략(쉐이더/리소스팩은 적용). best-effort.
            ClientDefaults.ApplyAll(AppPaths.GameDir, endpoint, progress, registerServer: channel.AutoConnect);
            ...
            // (5)+(6) 로더 설치 + endpoint(quickPlay) 주입 실행 (토큰은 직전 인증/재검증에서 확보)
            //   비-정식: autoConnect:false 로 quickPlay 생략(메인 메뉴 실행).
            var game = await _minecraft.LaunchAsync(session, endpoint, progress, ct, autoConnect: channel.AutoConnect).ConfigureAwait(false);
