...
| 모드 | 방법 | 필요한 것 | 서버 |
|------|------|-----------|------|
| **온라인 (직접 로그인)** | **시스템 브라우저** (요즘 공식 런처와 동일) | Azure 앱 client ID **1개**(메인테이너만) | `online-mode=true` |
| **오프라인** | 닉네임만 | 없음 | `online-mode=false` |
...
> client ID 가 설정되면 런처가 **자동으로 온라인(직접 로그인)을 기본**으로 켭니다("오프라인 모드" 체크가 기본 해제). 미설정이면 오프라인이 기본.

## 4단계 — 사용 (시스템 브라우저 직접 로그인)

1. 런처에서 **"오프라인 모드" 체크 해제**(client ID 설정 시 기본 해제됨).
2. **Play** → **기본 브라우저가 자동으로 열려** MS 로그인 페이지 표시.
3. 원하는 계정으로 로그인 → 브라우저가 "로그인 완료" → **런처로 자동 복귀**(코드 입력 없음).
