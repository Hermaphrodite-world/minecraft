...
> **Minecraft 26.1.2 / Fabric** · 77 모드 + 4 쉐이더팩 + 6 리소스팩 + 한국어 번역팩.
> 대부분은 **자동으로 동작**하니 신경 쓸 필요 없고, 아래 ⭐ 표시된 것들만 **키/명령어로 직접 사용**합니다.
> 미번역 모드는 **한국어 번역 리소스팩**이 자동 적용돼 한국어로 표시됩니다(런처가 기본 활성).
> 🆕 **Ping Wheel** — 화면 보고 키를 누르면 위치/엔티티에 임시 마커(핑) + 소리(친구에게 표시).
